...
export class HexViewportManager {
    ...
    constructor(config: ViewportConfig, containers: PIXI.Container[], containerNode: HTMLElement) {
        this.config = {
            minZoomLevel: 0.1,
            panSpeed: 1,
            zoomSpeed: 0.001,
        ...
        // Use debounced callback for expensive operations like hex visibility updates
        this.debouncedViewportChange = debounce(() => {
            if (this.config.onViewportChange) {
                this.config.onViewportChange();
            }
        }, 200); // Increased delay to reduce computational overhead during rapid movements
    ...
    private handlePanStart(event: PointerEvent): void {
        ...
        // - Left-click (button 0) when in select mode
        const isLeftClick = event.button === 0;
        const isMiddleOrRightClick = event.button === 1 || event.button === 2;
        const canPan = isMiddleOrRightClick || (isLeftClick && this.currentTool === 'select');
    ...
    private handlePanEnd(_event: PointerEvent): void {
        if (this.state.isPanning) {
            ...
            // This ensures hex visibility is updated promptly when user stops panning
    ...
    centerOn(worldX: number, worldY: number, screenWidth: number, screenHeight: number): void {
        if (screenWidth <= 0 || screenHeight <= 0) {
            console.warn('[HexViewportManager] Invalid screen dimensions, cannot center properly');
            return;
        }
